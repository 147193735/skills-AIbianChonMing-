...
/** 脚手架清单只写 MyFeatures，禁止落入 cqone 工程（易误提交 SVN） */
const MY_FEATURES_ROOT = process.env.MY_FEATURES
    || path.join(os.homedir(), "Desktop", "MyFeatures");
const MY_FEATURES_CHECKLISTS = path.join(MY_FEATURES_ROOT, "checklists");
...
function writeChecklist(names, scope) {
    return `# ${names.pascal} 模块 — 手动注册清单
# 生成时间: ${today()}
# 禁止写入 cqone 工程目录（易误提交 SVN）；本文件仅存于 MyFeatures/checklists/
# 模块: ${scope}/${names.folder}
...
function main() {
    ...
    if (args.mode === "full") {
        ...
        // 清单只进 MyFeatures/checklists，绝不写入工程模块目录
        ensureDir(MY_FEATURES_CHECKLISTS, args.dryRun);
        const checklistPath = path.join(MY_FEATURES_CHECKLISTS, `${names.folder}.scaffold-checklist.txt`);
        writeFile(checklistPath, writeChecklist(names, args.scope), { ...opts, force: true });
        console.log(`  清单(MyFeatures): ${checklistPath}`);
    }
