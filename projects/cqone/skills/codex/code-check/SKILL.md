...
description: >-
  ...
  existing common utilities are reused instead of reinvented; identifies
  dialogue-tip comments (坑/注意/需手动…) in the checked scope. Use when the
...
2. 确定范围（路径或 diff）；若已配置 `codegraph_explore`，优先用它拉模块实现与调用关系；否则用 `rg` 和文件读取完成同等跟链。
3. 按下方清单逐项检查；对「重复调用 / 缺防护 / 未复用」类结论，**必须跟到 callee 实现**再定论（见「跟链防误报」）。
4. 只报**可行动**问题（有位置 + 原因 + 建议）。
5. 标记对话提示型注释（见清单 F）；仅在用户明确要求一并修改时才删除，并在报告中说明。
6. 输出固定报告格式；默认只报告，不改代码。
...
| 场景 | 动作 |
|------|------|
| `playLoop` / `setUrl` / `play` / loader | 跟到 `MvArea`→`UiMv.setUrl` 等实现；同 url 已 early-return 则**不报** |
| `setData` / `refresh` / `numItems` | 跟到列表/组件是否同数据跳过或虚拟复用 |
| `eventLater` / `runLater` / `RedDotTask` | 确认是否队列去重，再谈「重复刷」 |
| 「手写了 X，该用 Y」 | 查实现或搜索确认 Y 的 API 与语义匹配后再报复用 |
...
判定与禁写规则见 [code-standards](../code-standards/SKILL.md)「禁止对话提示型注释」。默认报告；用户明确要求一并修改时才删除。

- [ ] 扫范围内 `//` / `/** */`：若主要在提醒「注意 / 需手动 / 记得 / FGUI 坑 / 协作备忘」，而非说明职责或业务原因 → 标记为待删除
- [ ] 若用户要求修改且删除后提示仍有协作价值 → 在报告中复述要点
- [ ] 勿借机补全其它注释或改排版；只在明确授权时删除提示型注释
