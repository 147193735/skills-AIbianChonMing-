...
**触及底层必读源码**：分析/排障落到引擎或 FGUI API（`bin/libs`、遮罩、控制器、显示对象、渲染等）时，遵循 skill `laya-fgui-engine-source`：有 `codegraph` 时优先用它，否则用 `rg` 和文件读取查 `C:/myPro/LayaAir-LayaAir_2.13.1` 与 `C:/myPro/FairyGUI-layabox-master`，勿只猜业务封装或压缩 JS。
