...
description: >-
  ...
  TypeScript sources (via codegraph MCP preferred) when analyzing problems or
...
| 库 | 路径 | 说明 | codegraph |
|---|---|---|---|
| LayaAir 2.13.1 | `C:/myPro/LayaAir-LayaAir_2.13.1` | 引擎源码；业务逻辑多在 `src/` | 已索引，可用 |
| FairyGUI-layabox | `C:/myPro/FairyGUI-layabox-master` | FGUI for Laya；实现多在 `source/` | 已索引，可用 |
...
1. **优先用 codegraph MCP**（`user-codegraph` → `codegraph_explore`）在上表两个仓库查符号/调用链；一次查询拿源码 + 调用路径，再按需 Grep/Read 补细节。不要先糊 `fairygui.min.js` / `laya.*.js`。
