...
- **grill-me**：设计压力测试技能，提到"grill me / 考我 / 压测设计"时触发；提问用 `vscode_askQuestions`（互动勾选）。Cursor 侧对应工具为 `AskQuestion`。
...
- **code-standards**：项目代码规范（命名、注释、顺序、编码约定），编辑 `.ts`/`.tsx` 文件时**局部触发**，仅在 TypeScript 文件中生效
...
当用户想要对一个计划或设计进行压力测试，或者提到"grill me"/"考我"/"压测设计"时，启用以下行为（普通「有疑问追问」不走本技能）：
...
每次提问必须使用上表对应工具（互动勾选），禁止纯文字列表冒充选择题：

1. **提供选项**：提供 2-4 个合理的预设选项。
2. **末项自定义**：始终添加 `✏️ 其他 / 自定义回答`；VS Code 保持 `allowFreeformInput: true`。
3. **多选支持**：VS Code 用 `multiSelect: true`；Cursor 用 `allow_multiple: true`。
4. **推荐答案**：VS Code 用 `recommended: true`；Cursor 在 label 标 `⭐ 推荐`。
...
## 第六步：配置 `code-standards.instructions.md`

> ✅ 已自动配置完成，无需手动操作。

本项目归档将 `skills/cursor/code-standards/code-standards.instructions.md` 复制到全局 prompts 目录。

该指令文件使用 `applyTo: "**/*.{ts,tsx}"`，因此**仅在编辑 `.ts`/`.tsx` 文件时触发**，不影响其他文件类型的代码生成。

### 验证方式

检查 `%APPDATA%\Code\User\prompts\` 目录下是否存在 `code-standards.instructions.md`：
